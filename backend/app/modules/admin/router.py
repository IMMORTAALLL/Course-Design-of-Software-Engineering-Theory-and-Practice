...
def sensitive_word_list(db: Session = Depends(get_db)):
...
def audit_queue_decision(item_id: int, payload: AuditDecisionIn, db: Session = Depends(get_db)):
...
def report_decision(item_id: int, payload: ReportDecisionIn, db: Session = Depends(get_db)):
...
def sensitive_word_toggle(word_id: int, payload: SensitiveWordToggleIn, db: Session = Depends(get_db)):
...
def user_moderation_records(db: Session = Depends(get_db)):
